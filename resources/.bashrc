...
if [ -f ~/.provision_done ]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        bash ~/provision.sh
        # create .provision_done file
        touch ~/.provision_done
        echoGreen "✅ Provisioning effectué avec succès. Vous pouvez utiliser votre shell."
    else
        echoRed "⚠️  Provisioning non effectué. Certains outils peuvent manquer."
    fi
fi
